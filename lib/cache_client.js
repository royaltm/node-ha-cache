...
const debug = require('debug')('ha-cache:client');
...
const CLIENT_MESSAGE_EXPIRATION_MS = 5000;
...
class CacheClient extends ZmqDealerSocket {
  ...
  **/
  constructor(urls, options) {
    ...
    debug('default request time-out: %s ms', this.defaultReqTimeoutMs);
    var discoveryinterval = +this.options.discoveryinterval;
    if (isFinite(discoveryinterval)) {
      if (discoveryinterval < 0) {
        debug('cluster urls discovery once at startup only');
        ...
        debug('cluster urls discovery once in %d ms', discoveryinterval);
        this.refreshServerUrls().then(setupDiscovery, setupDiscovery);
      }
    }
  }
  ...
  refreshServerUrls() {
    return this.request([CMD_DISCOVER, uint2bin(now() + CLIENT_MESSAGE_EXPIRATION_MS)]
  ...
  get(key, after) {
    var expire = now() + this.defaultReqTimeoutMs;
    var cmd = [CMD_GET, undefined, key];
    if (after) {
      cmd.push(uint2bin(after));
    }
    var request = () => {
      cmd[1] = uint2bin(now() + CLIENT_MESSAGE_EXPIRATION_MS);
      return this.request(cmd)
      .then(([type, value, expire]) => {
        switch(type[0]) {
          case RES_VALUE_MATCH:
            return {value: value, expire: bin2uint(expire)};
