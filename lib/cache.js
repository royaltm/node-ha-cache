...
const debug = require('debug')('ha-cache:server');
...
const STATE_SOURCING  = Symbol('sourcing');
  ...
  **/
  constructor(options) {
    ...
    outgoing.bind(this.bindUrl, err => {
      ...
      debug('%s broadcasting on: %s cluster peers: %s majority: %s', pid, this.bindUrl, apiUrls.size, this.majority);
    ...
    server.bind(this.bindApiUrl, err => {
      if (err) return this.emit('error', err);
      debug('%s serving on: %s cache ttl: %s', pid, this.bindApiUrl, this.ttl);
    });
  ...
  storageGetOrCreateMeta.call(this, key).then(entry => {
    if (entry.expire > after) {
      ...
      this.dataStorage.get(key).then(value => {
        debug('%s >>> CLIENT %s VALUE: %s EXP: %s NOW: %s', pid, keystr, value.length, entry.expire, moment);
