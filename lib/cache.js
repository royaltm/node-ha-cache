...
function peerHandler(target, peer, type, key, term, ...data) {
  ...
  try {
    switch(type[0]) {
      ...
      case ANSWER_MATCH:
        term = bin2uint(term);
        debug('%s got ANSWER: %s key: %s /%s', pid, peer, key, term);
        return processAnswer.call(this, peer, peerKey, key, term, ...data);
      case ENTRYREQ_MATCH:
        debug('%s got ENTRYREQ: %s key: %s', pid, peer, key);
        return processEntryRequest.call(this, peer, key);
      case UPDATE_MATCH:
        term = bin2uint(term);
        debug('%s got UPDATE: %s key: %s /%s', pid, peer, key, term);
        return processUpdate.call(this, peer, key, term, ...data);
      case ANNOUNCE_MATCH:
        term = bin2uint(term);
        debug('%s got ANNOUNCE: %s key: %s /%s', pid, peer, key, term);
        return processAnnouncement.call(this, peer, key, term, ...data);
      case PING_MATCH:
      ...
      default:
        debug('%s got [%s]: %s %s %s', pid, type, target, key, bin2uint(term));
    }
  } catch (err) {
    this.emit('error', err);
  }
...
function processAnswer(peer, peerKey, key, term, vote) {
  var entry;
  ...
  if (state !== undefined && state.type === STATE_CANDIDATE) {
    ...
    try {
      vote = msgpack.decode(vote);
      if (!vote || 'object' !== typeof vote || 'number' !== typeof vote.expire || 'boolean' !== typeof vote.vote) {
        debug('invalid vote');
        return;
      }
    } catch(err) {
      debug('error decoding vote');
      return;
    }
    debug('new vote: %j ... %s terms: %s/%s', vote, state.type, entry.term, term);
    /* find best source of expired entry to fetch */
    if (vote.expire > state.reqExpire) {
      state.reqPeer = peer;
      state.reqExpire = vote.expire;
    }
    if (vote.expire > now()) {
      ...
      debug('%s: ON OUR TERM: %s  %s ? %s', pid, vote.vote, state.agreed + vote.vote, this.majority);
      if (vote.vote && ++state.agreed >= this.majority) {
        /* start sourcing */
...
function processQuestion(peer, key, term) {
  ...
  var sendAnswer = (entry) => {
    ...
    this.outgoing.send([peer, this.myIdentity, ANSWER, key,
                              uint2bin(entry.term), msgpack.encode({expire: entry.expire, vote: vote})]);
  };
  if (state === undefined) {
    /* 1.1. idle */
