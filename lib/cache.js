  ...
  var timeoutcb = state.timeoutcb = () => {
    ...
    if (ttl > 0) {
      ...
      if (votes.size + 1 < this.majority) {
        ...
        ++entry.term;
        this.storage.set(entry.key, entry).then(() => {
...
function startSourcing(keystr, entry) {
  ...
  Promise.resolve(this.source(key)).then(value => {
    entry.value = value;
    entry.expire = now() + this.ttl;
    return this.storage.set(key, entry);
  })
  .then(entry => {
    sendEntryToClients.call(this, keystr, entry);
    /* send update */
...
function processEntryRequest(peer, key) {
  this.storage.get(key).then(entry => {
...
function processUpdate(peer, key, term, expire, value) {
  ...
  (state !== undefined
           ...
           : storageGetOrCreate.call(this, key))
  .then(entry => {
    ...
    return shouldUpdate ? this.storage.set(key, entry)
                        : entry;
  })
  .then(entry => {
...
function processQuestion(peer, key, term) {
  ...
  if (state === undefined) {
    /* 3.1. idle */
    storageGetOrCreate.call(this, key).then(entry => {
      // be safe, it's async
      if (this._states.has(keystr)) {
        ...
        if (term > entry.term) {
          entry.term = term;
          return this.storage.set(key, entry).then(sendAnswer);
        }
        else
          sendAnswer(entry);
      }
    ...
    if (term > entry.term) {
      entry.term = term;
      this.storage.set(key, entry).then(sendAnswer)
      .catch(err => {
        /* this is fatal, so better die */
        this.emit('error', err);
      });
...
function storageGetOrCreate(key) {
  return this.storage.get(key).then(entry => (entry || {key: key, term: 0, expire: 0}));
}
...
function clientHandler(ident, reqid, command, expire, key, after) {
  ...
  storageGetOrCreate.call(this, key).then(entry => {
    if (entry.expire > after) {
      ...
      this.server.send([ident, reqid, RES_VALUE, entry.value, uint2bin(entry.expire)]);
    }
    else {
    ...
    if (entry.expire < moment && !this._states.has(keystr)) {
      /* 2.3 we have expired entry or none so ask QUESTION */
      if (this.majority === 1) {
      /* single peer */
        startSourcing.call(this, keystr, entry)
      }
...
function sendEntryToClients(keystr, entry) {
  var clientRequests = this._clientRequests;
  var queue = clientRequests.fetch(keystr);
  if (queue !== undefined) {
    var server = this.server;
    var {value, expire} = entry;
    var expbin = uint2bin(expire);
