...
module.exports = exports = function processAnswer(peer, peerKey, key, term, expire, vote) {
  ...
  if (state !== undefined && state.type === STATE_CANDIDATE) {
    ...
    if (expire > now()) {
      debug('peer %s has fresh entry: %j', peerKey, keystr);
      ...
      debug('peer %s vote on our term: %s agreed: %s/%s', peerKey, vote, state.agreed + vote, this.majority);
    ...
    /* update term if necessary */
      ...
      debug('our term too low: %s < %s aborting %j', entry.term, term, keystr);
