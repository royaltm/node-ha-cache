...
class Cache extends EventEmitter {
  ...
  **/
    ...
    if (queue !== undefined) {
      ...
      for(var item of queue) {
        ...
        if (expire > after) {
          ...
          debug('>>> CLIENT VALUE %j size: %d', keystr, value.length);
          server.send(request);
        }
  ...
  _sendUpdate(target, entry, value) {
    debug('>>> %s UPDATE /%s exp: %s', target, entry.term, entry.expire);
