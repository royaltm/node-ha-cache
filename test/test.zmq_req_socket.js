...
const ZmqReqSocket = require('../lib/zmq_req_socket');
const zmq = require('zmq');

test('should be a function', t => {
  t.type(ZmqReqSocket, 'function');
  t.end();
});

test('rep', suite => {
  var [rep, url] = createZmqSocket('rep');

  suite.test('test requests', t => {
    t.plan(7);
    var socket = new ZmqReqSocket(url);
    return Promise.all([
      ...
      socket.request("foo").then(res => {
        ...
        t.type(res.length, 1);
        t.type(res[0], Buffer);
        t.strictEquals(res[0].toString(), "bar");
...
test('router', suite => {
  ...
  suite.test('test request timeout', t => {
    t.plan(4);
    var socket = new ZmqReqSocket(url, 100);
    var start = Date.now();
